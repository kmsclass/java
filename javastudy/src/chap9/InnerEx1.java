 ...
 *   - 인스턴스중첩클래스 : 객체화시 외부클래스의 객체화 필요.
 *   - static중첩클래스 : new 외부클래스명.중첩클래스()
 *   - local중첩클래스  : 메서드 내부에서 선언된 클래스. 해당 메서드에서만 접근 가능.
 *   
 * 중첩클래스의 특징
 *  1. 참조자료형으로 사용됨. 객체화 가능. Object 클래스의 하위클래스. 다른 클래스를 상속 가능.
 *     외부클래스명.중첩클래스
 *     중첩클래스별로 바이트코드 생성   
...
class Outer1 {
	...
	void method() {
		//method() 내부에서만 사용가능.
		class LocalInner extends Object {  //local 중첩클래스 : 메서드 내부에서 선언된 클래스. 
			int iv = 600;
			static int cv = 700;
