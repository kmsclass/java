...
//	static int cv3 = iv1;  //클래스변수 = 인스턴스변수
	static int cv3 = new MemberCall().iv1;//클래스 멤버에서 인스턴스멤버 호출시 객체화 필요
	...
	static void method2() { //클래스메서드
		System.out.println("cv1 + cv2="+(cv1+cv2)); //클래스변수 호출
//		System.out.println("iv1 + iv2="+(iv1+iv2)); //인스턴스호출 불가
		MemberCall m = new MemberCall();
		System.out.println("iv1 + iv2="+(m.iv1+m.iv2)); //인스턴스호출시 객체화 필요
	}
